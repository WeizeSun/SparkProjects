...
val positive = Array("like", "love", "good", "great", "happy", "cool", "congratulations", "cheers", "magnificent", "wonderful", "miracle", "proud", "bright", "strong")
val negative = Array("no", "not", "dislike", "bad", "hate", "disgust", "hopeless", "fear", "disaster", "disagree", "shame", "ashame", "oh no", "lie")
...
def creatingFunc(): StreamingContext = {
  ...
  val hashTagStream = twitterStream.map({line =>
    val count = line.getText.toLowerCase().split(" ").map({word =>
  ...
  windowedhashTagCountStream.foreachRDD(hashTagCountRDD => {
    val topEndpoints = hashTagCountRDD.top(3)(SecondValueOrdering)
